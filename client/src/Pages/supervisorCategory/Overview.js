...
function Overview() {
    ...
    const fetchPlayers = async () => {
        try {
            ...
            const response = await axios.post(`http://localhost:5000/api/player/category/${selectedSubCategory}`, null, {
                headers: {
            ...
            <ul className="player-list">
                {players.map((player, index) => (
                    <React.Fragment key={player._id}>
                        <li>
                            <img className="player-image" src={player.photo ? player.photo : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRPC7ikoXgW66ZFb0JFMRADmdD8hxYbgk2N5g&usqp=CAU'} alt="" />
